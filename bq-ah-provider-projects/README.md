...
1.  **Configure `setup.env`:**
    ...
    * Edit `setup.env` and provide values for all **mandatory settings**:
        ...
        * *(Workaround for IP Allowlist)*: Since the `s01` script doesn't process an IP allowlist variable from `setup.env`, you will need to manually edit `generated/terraform.auto.tfvars` later to add your IP for GKE control plane access.
